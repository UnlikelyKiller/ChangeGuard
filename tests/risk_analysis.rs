...
use changeguard::impact::packet::{ImpactPacket, ChangedFile, RiskLevel};
...
fn test_risk_analysis_integration() {
    ...
    assert!(packet.risk_reasons.iter().any(|r| r.contains("Public symbol modified")));
}
...
fn test_risk_analysis_high_volume() {
    ...
    // Add public symbols
    packet.changes[0].symbols = Some(vec![
        Symbol {
            name: "api".into(),
            kind: SymbolKind::Function,
            is_public: true,
