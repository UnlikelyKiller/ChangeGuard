...
use changeguard::state::graph_kinds::{NodeKind, EdgeKind};
...
fn test_entity_to_test_link() {
    ...
    storage.put_node_batch(&[symbol_node, test_node]).expect("Failed to put nodes");
    ...
    storage.put_edge_batch(&[edge]).expect("Failed to put edges");
