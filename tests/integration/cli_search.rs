...
fn test_search_fuzzy_fallback_and_hint() {
    ...
    assert!(stdout.contains("Falling back to fuzzy search"), "Expected fallback to fuzzy search: {}", stdout);
    ...
    assert!(stdout_json.contains(r#"record_kind":"fuzzy_match"#), "Expected JSON fallback record: {}", stdout_json);
