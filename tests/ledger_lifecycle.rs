...
fn setup_storage() -> StorageManager {
...
fn test_ledger_start_commit_roundtrip() {
    ...
    let tx_id = tx_mgr.start_change(TransactionRequest {
    ...
    // Verify pending
    let pending = tx_mgr.get_pending(entity).expect("Should find pending").expect("Should be Some");
    assert_eq!(pending.tx_id, tx_id);
    assert_eq!(pending.status, "PENDING");

    // Commit
    tx_mgr.commit_change(tx_id.clone(), CommitRequest {
    ...
    // Verify resolved
    let tx = tx_mgr.get_transaction(&tx_id).expect("Should find tx").expect("Should be Some");
    assert_eq!(tx.status, "RESOLVED");

    // Verify ledger entry exists
    let entries = tx_mgr.get_ledger_entries_for_tx(&tx_id).expect("Should find entries");
...
fn test_ledger_start_rollback() {
    ...
    tx_mgr.rollback_change(tx_id.clone()).expect("Should rollback");

    let tx = tx_mgr.get_transaction(&tx_id).expect("Should find tx").expect("Should be Some");
    assert_eq!(tx.status, "ROLLED_BACK");

    // Should be able to start a new one for same entity
    let new_tx_id = tx_mgr.start_change(TransactionRequest {
        category: Category::Bugfix,
        entity: entity.to_string(),
        ..Default::default()
...
fn test_ledger_conflict() {
    ...
    tx_mgr.start_change(TransactionRequest {
...
fn test_ledger_atomic() {
    ...
    let entity = "src/main.rs";
    tx_mgr.atomic_change(TransactionRequest {
...
fn test_fuzzy_matching() {
    ...
    let real_id = tx_mgr.start_change(TransactionRequest {
        category: Category::Feature,
