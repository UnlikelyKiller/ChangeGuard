...
fn test_verify_command_pass() {
    let cmd = "echo hello";
    let result = execute_verify(Some(cmd.into()), 5, false);
    assert!(result.is_ok());
}
...
fn test_verify_command_timeout() {
    ...
    let result = execute_verify(Some(cmd.into()), 1, false);
    assert!(result.is_err());
...
fn test_verify_command_not_found() {
    let result = execute_verify(Some("nonexistent_command_9999".into()), 5, false);
    assert!(result.is_err());
    let err_msg = format!("{:?}", result.err().unwrap());
