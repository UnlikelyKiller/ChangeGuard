...
fn migrate_ledger_entries(sqlite: &StorageManager, cozo: &CozoStorage) -> Result<()> {
    ...
    if !batch.is_empty() {
        let script = format!("?[id, tx_id, category, entry_type, entity_normalized, change_type, summary, reason, committed_at, is_breaking, verification_status, trace_id] <- {} :put ledger_entry", serde_json::to_string(&batch).into_diagnostic()?);
...
fn migrate_project_symbols(sqlite: &StorageManager, cozo: &CozoStorage) -> Result<()> {
    ...
    if !batch.is_empty() {
        let script = format!("?[id, file_path, qualified_name, symbol_name, symbol_kind, is_public, line_start, line_end] <- {} :put project_symbol", serde_json::to_string(&batch).into_diagnostic()?);
        cozo.run_script(&script)?;
...
mod tests {
    ...
    fn test_migration_parity() {
        ...
        // 3. Verify in Cozo
        let res = cozo.run_script("?[summary] := *ledger_entry{tx_id: 'tx_123', summary: summary}").unwrap();
