...
use crate::index::entrypoint::{detect_python_entrypoints, detect_rust_entrypoints, detect_typescript_entrypoints, EntrypointKind, EntrypointStats};
...
impl ProjectIndexer {
    ...
    pub fn new_for_worker(repo_path: Utf8PathBuf) -> Self {
        Self {
            storage: StorageManager::init_from_conn(rusqlite::Connection::open_in_memory().unwrap()),
            repo_path,
        }
    }
    ...
    pub fn index_file(&self, path: &Utf8Path) -> Result<(ProjectFile, Vec<ProjectSymbol>)> {
        ...
        let mut pf = ProjectFile {
            ...
            language: relative.extension().and_then(|ext| Language::from_extension(ext)).map(|l| format!("{:?}", l)),
            content_hash: None,
            git_blob_oid: None,
            file_size: fs::metadata(path).ok().map(|m| m.len() as i64),
            mtime_ns: None,
            parser_version: PARSER_VERSION.to_string(),
            parse_status: if outcome.analysis_status.symbols == crate::impact::packet::AnalysisStatus::Ok { "OK".to_string() } else { "PARSE_FAILED".to_string() },
        ...
        let ps = outcome.symbols.unwrap_or_default()
            .into_iter()
    ...
    pub fn full_index(&mut self) -> Result<IndexStats> {
        ...
        Ok(IndexStats { duration_ms, ..stats })
    }
    ...
    pub fn incremental_index(&mut self) -> Result<IndexStats> {
        ...
        for file_path in &current_files {
            let relative = file_path.strip_prefix(&self.repo_path).unwrap_or(file_path).to_string();
        ...
        if files_to_reindex.is_empty() {
             return Ok(IndexStats { duration_ms: start.elapsed().as_millis() as u64, files_indexed: 0, symbols_indexed: 0, parse_failures: 0, skipped_binary: 0, skipped_unsupported: 0 });
        }
        ...
        let rx = pool.process_parsing(files_to_reindex, Some(pb.clone()), move |path| {
            ...
            let ps = outcome.symbols.unwrap_or_default().into_iter().map(|s| symbol_to_project_symbol(&s, 0, &now)).collect();
            Ok((pf, ps))
        })?;
        ...
        Ok(IndexStats { duration_ms: start.elapsed().as_millis() as u64, ..stats })
    }

    fn collect_results(&mut self, rx: crossbeam::channel::Receiver<JobResult>, is_full: bool) -> Result<IndexStats> {
        ...
        while let Ok(result) = rx.recv() {
            match result {
                JobResult::Parsed(pf, ps) => {
                    if pf.parse_status == "PARSE_FAILED" { parse_failures += 1; }
                    else { files_indexed += 1; }
                    ...
                    if batch_files.len() >= BATCH_SIZE {
                        if is_full { self.insert_batch(&batch_files, &batch_symbols)?; }
                        else { self.upsert_batch(&batch_files, &batch_symbols)?; }
                        batch_files.clear();
                        batch_symbols.clear();
                    }
                }
        ...
        Ok(IndexStats { files_indexed, symbols_indexed, parse_failures, skipped_binary: 0, skipped_unsupported: 0, duration_ms: 0 })
    }

    pub fn discover_files(&self) -> Result<Vec<Utf8PathBuf>> {
        RepoWalker::new(self.repo_path.clone(), SUPPORTED_EXTENSIONS, BINARY_EXTENSIONS).discover_files()
    }

    pub fn discover_doc_files(&self) -> Result<Vec<Utf8PathBuf>> {
        RepoWalker::new(self.repo_path.clone(), SUPPORTED_EXTENSIONS, BINARY_EXTENSIONS).discover_doc_files()
    }
    ...
    pub fn index_docs(&mut self) -> Result<DocIndexStats> {
        ...
        for doc_path in &doc_files {
            let relative_path = doc_path.strip_prefix(&self.repo_path).unwrap_or(doc_path).to_string();
            let content = match fs::read_to_string(doc_path) {
                Ok(c) => c,
                Err(e) => { warn!("Failed to read doc file {}: {}", doc_path, e); parse_failures += 1; continue; }
            };
            let parsed = parse_markdown(&content, &relative_path);
            let file_id = self.ensure_file_entry(&relative_path, &content, &now)?;
            let sections_json = serde_json::to_string(&parsed.sections).unwrap_or_else(|_| "[]".to_string());
            let code_blocks_json = serde_json::to_string(&parsed.code_blocks).unwrap_or_else(|_| "[]".to_string());
            let internal_links_json = serde_json::to_string(&parsed.internal_links).unwrap_or_else(|_| "[]".to_string());
        ...
        Ok(DocIndexStats { docs_indexed, parse_failures, missing_readme: !has_readme })
    }

    fn ensure_file_entry(&mut self, relative_path: &str, content: &str, now: &str) -> Result<i64> {
        let content_hash = blake3::hash(content.as_bytes()).to_hex().to_string();
        let conn = self.storage.get_connection();
        let existing_id: Option<i64> = conn.query_row("SELECT id FROM project_files WHERE file_path = ?1", [relative_path], |row| row.get(0)).ok();
        if let Some(id) = existing_id { return Ok(id); }
        let conn = self.storage.get_connection_mut();
    ...
    pub fn index_topology(&mut self) -> Result<TopologyIndexStats> {
        ...
        for file_path in &all_files {
            ...
            if let Some(parent) = relative.parent() {
                ...
                if !dir.is_empty() { dir_files.entry(dir).or_default().push(relative.to_string()); }
            }
        }
        let mut all_dirs: HashSet<String> = dir_files.keys().cloned().collect();
        for dir in dir_files.keys() {
            let mut current = dir.as_str();
            while let Some(parent) = std::path::Path::new(current).parent().and_then(|p| p.to_str()) {
                if !parent.is_empty() && !all_dirs.contains(parent) { all_dirs.insert(parent.to_string()); }
                current = parent;
            }
        }
        ...
        for dir_path in &all_dirs {
            let files: Vec<&str> = dir_files.get(dir_path).map(|v| v.iter().map(|s| s.as_str()).collect()).unwrap_or_default();
        ...
        Ok(TopologyIndexStats { directories_classified, unclassified, role_counts })
    }
    ...
    pub fn classify_entrypoints(&mut self) -> Result<EntrypointStats> {
        ...
        let rows: Vec<(i64, i64, String, String, bool, Option<String>)> = stmt.query_map([], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?, row.get::<_, i32>(4)? != 0, row.get(5)?))).into_diagnostic()?.collect::<Result<Vec<_>, _>>().into_diagnostic()?;
        drop(stmt);

        let mut file_symbols: HashMap<i64, Vec<(i64, String, String, bool, Option<String>)>> = HashMap::new();
        for (id, file_id, name, kind, is_public, metadata) in &rows { file_symbols.entry(*file_id).or_default().push((*id, name.clone(), kind.clone(), *is_public, metadata.clone())); }

        let mut path_stmt = conn.prepare("SELECT id, file_path, language FROM project_files").into_diagnostic()?;
        let path_rows: Vec<(i64, String, Option<String>)> = path_stmt.query_map([], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?))).into_diagnostic()?.collect::<Result<Vec<_>, _>>().into_diagnostic()?;
        drop(path_stmt);

        let mut file_paths: HashMap<i64, String> = HashMap::new();
        for (id, path, _lang) in &path_rows { file_paths.insert(*id, path.clone()); }

        let mut stats = EntrypointStats::default();
        let now = chrono::Utc::now().to_rfc3339();

        for (file_id, symbols) in &file_symbols {
            let file_path = match file_paths.get(file_id) { Some(p) => p.clone(), None => continue };
            let file_lang = path_rows.iter().find(|(id, _, _)| id == file_id).and_then(|(_, _, lang)| lang.clone());
            let full_path = self.repo_path.join(&file_path);
            let Ok(content) = fs::read_to_string(&full_path) else { continue };
            
            let sym_vec: Vec<crate::index::symbols::Symbol> = symbols.iter().map(|(_, name, kind, is_public, metadata)| crate::index::symbols::Symbol {
                name: name.clone(), kind: crate::index::symbols::SymbolKind::parse(kind).unwrap_or(crate::index::symbols::SymbolKind::Function), is_public: *is_public, cognitive_complexity: None, cyclomatic_complexity: None, line_start: None, line_end: None, qualified_name: None, byte_start: None, byte_end: None, entrypoint_kind: None,
                metadata: metadata.as_ref().and_then(|m| serde_json::from_str(m).ok()).unwrap_or_default(),
            }).collect();

            let classifications = match file_lang.as_deref() {
                Some("Rust") => detect_rust_entrypoints(&content, &sym_vec),
                Some("TypeScript") | Some("JavaScript") => detect_typescript_entrypoints(&content, &sym_vec, &file_path),
            ...
            for class in &classifications {
                let db_id = symbols.iter().find(|(_, name, _, _, _)| name == &class.symbol_name).map(|(id, _, _, _, _)| *id);
                if let Some(id) = db_id {
                    ...
                    match class.kind { EntrypointKind::Entrypoint => stats.entrypoints += 1, EntrypointKind::Handler => stats.handlers += 1, EntrypointKind::PublicApi => stats.public_apis += 1, EntrypointKind::Test => stats.tests += 1, EntrypointKind::Ffi => stats.ffi += 1, EntrypointKind::Macro => stats.macros += 1, EntrypointKind::Internal => stats.internal += 1 }
                }
            }
        }
        Ok(stats)
    ...
    pub fn clear_routes(&self, file_ids: &[i64]) -> Result<()> {
        RouteExtractor::new(&self.storage, self.repo_path.as_std_path().to_path_buf()).clear_routes(file_ids)
    }

    pub fn clear_structural_edges(&self, file_ids: &[i64]) -> Result<()> {
        if file_ids.is_empty() { return Ok(()); }
        let conn = self.storage.get_connection();
        for &fid in file_ids { conn.execute("DELETE FROM structural_edges WHERE caller_file_id = ?1", [fid]).into_diagnostic()?; }
        Ok(())
    }
    ...
    pub fn clear_data_models(&self, file_ids: &[i64]) -> Result<()> {
        DataModelExtractor::new(&self.storage, self.repo_path.as_std_path().to_path_buf()).clear_data_models(file_ids)
    }

    pub fn extract_observability(&self) -> Result<crate::index::observability::ObservabilityStats> {
        ObservabilityExtractor::new(&self.storage, self.repo_path.as_std_path().to_path_buf()).extract()
    }
    ...
    pub fn get_all_call_edges(&self) -> Result<Vec<crate::index::call_graph::CallEdge>> {
        ...
        let edges = stmt.query_map([], |row| Ok(CallEdge {
            caller_name: row.get(0)?, caller_file: PathBuf::from(row.get::<_, String>(1)?), callee_name: row.get::<_, Option<String>>(2)?.unwrap_or_default(), callee_file: row.get::<_, Option<String>>(3)?.map(PathBuf::from),
            call_kind: match row.get::<_, String>(4)?.as_str() { "METHOD_CALL" => CallKind::MethodCall, "TRAIT_DISPATCH" => CallKind::TraitDispatch, "DYNAMIC" => CallKind::Dynamic, "EXTERNAL" => CallKind::External, _ => CallKind::Direct },
            resolution_status: match row.get::<_, String>(5)?.as_str() { "AMBIGUOUS" => ResolutionStatus::Ambiguous, "UNRESOLVED" => ResolutionStatus::Unresolved, "CAPPED" => ResolutionStatus::Capped, _ => ResolutionStatus::Resolved },
            confidence: row.get(6)?, evidence: row.get(7)?,
    ...
    pub fn infer_services(&mut self) -> Result<ServiceIndexStats> {
        ...
        let (routes, data_models, call_graph) = {
            ...
            let routes: Vec<ApiRoute> = route_stmt.query_map([], |row| Ok(ApiRoute { method: row.get(0)?, path_pattern: row.get(1)?, handler_symbol_name: row.get(2)?, framework: row.get(3)?, route_source: row.get(4)?, mount_prefix: row.get(5)?, is_dynamic: row.get::<_, i32>(6)? != 0, route_confidence: row.get(7)?, evidence: row.get(8)? })).into_diagnostic()?.collect::<Result<Vec<_>, _>>().into_diagnostic()?;
            ...
            let data_models: Vec<DataModelSource> = dm_stmt.query_map([], |row| Ok(DataModelSource { model: DataModel { model_name: row.get(0)?, model_kind: row.get(1)?, confidence: row.get(2)?, evidence: row.get(3)? }, source_path: row.get(4)? })).into_diagnostic()?.collect::<Result<Vec<_>, _>>().into_diagnostic()?;
            let call_graph = CallGraph { edges: self.get_all_call_edges()? };
            (routes, data_models, call_graph)
        };

        let topology = DirectoryTopology { classifications: self.storage.get_directory_classifications().unwrap_or_default() };
        ...
        tx.execute("UPDATE project_files SET service_name = NULL", []).into_diagnostic()?;
        let mut sorted_services = services.clone();
        sorted_services.sort_by(|a, b| b.directory.components().count().cmp(&a.directory.components().count()));
        ...
        Ok(ServiceIndexStats { services_inferred: services.len(), files_assigned })
    }
    ...
    fn clear_project_data(&mut self) -> Result<()> {
        let conn = self.storage.get_connection_mut();
        for table in ["symbol_centrality", "structural_edges", "api_routes", "data_models", "observability_patterns", "test_mapping", "ci_gates", "env_references", "env_declarations", "project_docs", "project_topology", "project_symbols", "project_files"] {
            conn.execute(&format!("DELETE FROM {}", table), []).into_diagnostic()?;
        }
        Ok(())
    }

    fn insert_batch(&mut self, files: &[ProjectFile], symbols: &[Vec<ProjectSymbol>]) -> Result<()> {
        let conn = self.storage.get_connection_mut();
        let tx = conn.unchecked_transaction().into_diagnostic()?;
        for (i, pf) in files.iter().enumerate() {
            insert_file_row(&tx, pf)?;
            let file_id = tx.last_insert_rowid();
            for ps in &symbols[i] { insert_symbol_row(&tx, ps, file_id)?; }
        }
        tx.commit().into_diagnostic()
    }

    fn upsert_batch(&mut self, files: &[ProjectFile], symbols: &[Vec<ProjectSymbol>]) -> Result<()> {
        let conn = self.storage.get_connection_mut();
        let tx = conn.unchecked_transaction().into_diagnostic()?;
        for (i, pf) in files.iter().enumerate() {
    ...
    fn store_index_metadata(&mut self) -> Result<()> {
        let conn = self.storage.get_connection_mut();
        let now = chrono::Utc::now().to_rfc3339();
        for (key, value) in [("parser_version", PARSER_VERSION), ("last_indexed_at", &now), ("index_version", "1"), ("schema_version", "1")] {
            conn.execute("INSERT OR REPLACE INTO index_metadata (key, value) VALUES (?1, ?2)", (key, value)).into_diagnostic()?;
        }
        Ok(())
    }
    ...
    fn load_existing_files(&self) -> Result<HashMap<String, ProjectFile>> {
        ...
        let rows = stmt.query_map([], |row| Ok(ProjectFile {
            id: Some(row.get(0)?), file_path: row.get(1)?, language: row.get(2)?, content_hash: row.get(3)?, git_blob_oid: row.get(4)?, file_size: row.get(5)?, mtime_ns: row.get(6)?, parser_version: row.get(7)?, parse_status: row.get(8)?, last_indexed_at: row.get(9)?,
        })).into_diagnostic()?.collect::<Result<Vec<_>, _>>().into_diagnostic()?;
        Ok(rows.into_iter().map(|pf| (pf.file_path.clone(), pf)).collect())
    }
...
fn create_progress_bar(total: usize) -> ProgressBar {
    ...
    pb.set_style(ProgressStyle::with_template("Indexing: {pos}/{len} files... {spinner}").unwrap_or_else(|_| ProgressStyle::with_template("{pos}/{len}").unwrap()));
    pb
}
...
pub fn get_file_id_by_path(conn: &Connection, file_path: &str) -> Result<i64> {
    conn.query_row("SELECT id FROM project_files WHERE file_path = ?1", [file_path], |row| row.get(0)).into_diagnostic()
}
...
pub fn delete_file_index_dependents(conn: &Connection, file_path: &str) -> Result<()> {
    ...
    for statement in [
        format!("DELETE FROM symbol_centrality WHERE file_id IN ({file_id_subquery}) OR symbol_id IN ({symbol_id_subquery})"),
        format!("DELETE FROM structural_edges WHERE caller_file_id IN ({file_id_subquery}) OR callee_file_id IN ({file_id_subquery}) OR caller_symbol_id IN ({symbol_id_subquery}) OR callee_symbol_id IN ({symbol_id_subquery})"),
        format!("DELETE FROM api_routes WHERE handler_file_id IN ({file_id_subquery}) OR handler_symbol_id IN ({symbol_id_subquery})"),
        ...
        format!("DELETE FROM test_mapping WHERE test_file_id IN ({file_id_subquery}) OR tested_file_id IN ({file_id_subquery}) OR test_symbol_id IN ({symbol_id_subquery}) OR tested_symbol_id IN ({symbol_id_subquery})"),
        format!("DELETE FROM ci_gates WHERE ci_file_id IN ({file_id_subquery})"),
        format!("DELETE FROM env_references WHERE file_id IN ({file_id_subquery}) OR symbol_id IN ({symbol_id_subquery})"),
