...
- [ ] Task 1.1: Extend `src/index/call_graph.rs` with `enumerate_call_chains(max_depth)`.
- [ ] Task 1.2: Implement cycle detection (visited-set tracking).
- [ ] Task 1.3: Filter external/stdlib nodes from chains.
...
- [ ] Task 2.1: Implement `compute_data_flow_coupling()` in `src/coverage/dataflow.rs`.
- [ ] Task 2.2: Implement 20% change-percentage threshold.
- [ ] Task 2.3: Implement SQL table-name model resolution fallback.
- [ ] Task 2.4: Enforce minimum chain depth of 2.
- [ ] Task 2.5: Write test: route+handler+model all changed → DataFlowMatch returned.
- [ ] Task 2.6: Write test: only route changed → no match (below 20%).
- [ ] Task 2.7: Write test: 3/10 nodes changed → 30% → match.
- [ ] Task 2.8: Write test: chain depth 1 (handler only) → skipped.

### Phase 3: Types
- [ ] Task 3.1: Define `CallChain`, `CallChainNode`, `DataFlowMatch` types.
- [ ] Task 3.2: Implement `Ord` for `DataFlowMatch` (by `change_pct` descending).
- [ ] Task 3.3: Add `data_flow_matches: Vec<DataFlowMatch>` to `ImpactPacket`.
- [ ] Task 3.4: Wire `finalize()` sort and `truncate_for_context()` clear.
...
- [ ] Task 4.1: Wire `compute_data_flow_coupling` into `execute_impact()` enrichment.
- [ ] Task 4.2: Implement risk weight: 4 per match, cap at 20.
