...
- `src/local_model/pruner.rs` [NEW] — Core pruning engine.
  ...
  - `prune_impact_fields(packet, mode) -> PrunedPacket` — drop observability/contracts/decisions that are irrelevant to the query mode (e.g., ReviewPatch mode doesn't need Prometheus signals).
...
- `src/retrieval/query.rs` — Reuse existing `query_embeddings` and cosine-similarity logic.
- `src/commands/ask.rs` — Wire pruner into the local backend path.

## Success Criteria
- Local backend token count never exceeds `config.local_model.context_window` (with 10% headroom).
...
- New unit tests for `pruner.rs` and updated `assemble_context` tests.
