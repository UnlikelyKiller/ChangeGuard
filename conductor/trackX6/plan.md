...
- [x] 1. Write integration test: create a ledger transaction that touched `src/commands/audit.rs`, run `changeguard ledger audit src/commands/audit.rs`, assert the transaction appears in output.
- [x] 2. Write unit test for `looks_like_file_path(s: &str) -> bool` helper: assert true for `src/foo.rs`, `C:\dev\foo.rs`, false for `my-service`.

## Phase 2 — Implementation
- [x] 3. Add `fn looks_like_file_path(s: &str) -> bool` in `src/commands/ledger_audit.rs`:
  - Returns `true` if `s` contains `/` or `\` or has an extension (`.` before last component).
- [x] 4. In `execute_ledger_audit`, after entity-name lookup:
...
- [x] 5. Add `LedgerDb::find_transactions_by_file(file_path: &str) -> Result<Vec<Transaction>>` in `src/ledger/db.rs`:
  ```sql
  ...
  ```
- [x] 6. Print a note when file-path results are included:
  ```rust
